"""
MoLoRAG+ v2 Evaluation Script
-----------------------------
This script evaluates the MoLoRAG+ system, which uses a fine-tuned LoRA adapter
for enhanced logical relevance scoring during document traversal.
"""
...
print(f"[*] Initializing MoLoRAG+ v2 on device: {DEVICE}")
...
# --- 2. Core Indexing & Traversal ---

class DocumentGraphIndex:
    """
    Handles PDF loading, CLIP embedding generation, and semantic graph construction.
    """
    ...
    def generate_embeddings(self):
        """Generates unit-normalized CLIP embeddings."""
        self.embeddings = []
        self.model.eval()
        with torch.no_grad():
    ...
    def build_graph(self):
        """Constructs a proximity graph based on cosine similarity."""
...
class MoLoRAGTraversal:
    """
    Traverses the page graph using both semantic and logical relevance scores.
    """
    ...
    def run_traversal(self, query):
        """Runs the multi-hop traversal for a given query."""
...
def calculate_metrics(prediction, ground_truth):
    """Computes Recall, Precision, NDCG, and MRR."""
...
def main():
    print("[*] Initializing MoLoRAG+ v2 Evaluation Engine...")
    ...
    for name, json_path, pdf_root in DATASETS:
        ...
        with open(json_path) as f:
            samples = json.load(f)[:5] # Run small slice
